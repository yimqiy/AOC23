...
def soln(m,mind,maxd):
    l = {}
    v = set()
    q = [(0, 0, 0, 0, 0)]  # Tuple (hl,no,de, distance, dir)
    while q:
        ...
        hlf=hlr=hll=hl
        r=[(1,0),(0,1),(-1,0),(0,-1)]
        for k in range(1,maxd-d+1):
            z = (k*r[n][0]+x,k*r[n][1]+y)
            if z[0]>=0 and z[1]>=0 and z[0]<xm and z[1]<ym:
                hlf += m[z[1]][z[0]]
                heappush(q,(hlf,z[0],z[1], d + k, n))
        if d>=mind:
            for k in range(1,maxd+1):
                z = (k*r[(n-1)%4][0]+x,k*r[(n-1)%4][1]+y)
                if z[0]>=0 and z[1]>=0 and z[0]<xm and z[1]<ym:
                    hll += m[z[1]][z[0]]
                    heappush(q,(hll,z[0],z[1], k, (n-1)%4))
                
                z = (k*r[(n+1)%4][0]+x,k*r[(n+1)%4][1]+y)
                if z[0]>=0 and z[1]>=0 and z[0]<xm and z[1]<ym:
                    hlr += m[z[1]][z[0]]
                    heappush(q,(hlr,z[0],z[1], k, (n+1)%4))
